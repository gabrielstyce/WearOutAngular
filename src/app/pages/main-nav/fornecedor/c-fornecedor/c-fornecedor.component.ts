...
export class CFornecedorComponent extends MasterEranca implements OnInit {
  ...
  @ViewChild('mytableProd', { static: true }) mytableProd: MatTable<any>;
      ...
      this.fornecedorService.getFornecedorById(this.fornecedor.userId).subscribe(
        (res: any) => {
          if (res) {
            if (res.userId) {
              this.fornecedor = res;
              this.fornecedorService.getProdutosFornecidos(res.userId).subscribe(resa => this.CarregarProdutos(resa as any[]));
            }
          } else {
  ...
  public CarregarProdutos(valores: any[]) {
    ...
    this.dataSourceProd.data.push(...valores);
    console.log('Logs' + valores);
    this.mytableProd.renderRows();
  }
